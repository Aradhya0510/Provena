...
# MAGIC Layer (SDOL)** using in-memory mock data from the fleet management domain.
...
# MAGIC The SDOL source tree must be synced to your workspace (see the `sdol_project_root` widget).
...
print(f"Provena version loaded -- exports: {sdol.__all__[:8]}...")
...
# MAGIC
# MAGIC SDOL defines **8 intent types** that describe WHAT the agent needs from data,
...
# MAGIC
# MAGIC SDOL connectors follow a three-tier architecture:
# MAGIC
...
# MAGIC
# MAGIC Every piece of data in SDOL carries a **ProvenanceEnvelope** -- a structured
...
# MAGIC ### Key Insight
# MAGIC
# MAGIC Every piece of data in SDOL carries a "birth certificate" -- you can always trace
...
# MAGIC When SDOL says "trust = 0.85", that is a deterministic calculation from
...
# MAGIC
# MAGIC SDOL detects these conflicts structurally by comparing `ContextElement` data
...
# MAGIC ### Key Insight
# MAGIC
# MAGIC An LLM MIGHT notice that two data sources disagree. SDOL ALWAYS detects it.
...
# MAGIC ### Key Insight
# MAGIC
# MAGIC This is the prompt fragment SDOL injects into the agent's system message. It gives
...
# MAGIC because SDOL gave it those numbers.
...
# MAGIC might not. SDOL wraps **deterministic guarantees** around the probabilistic core:
# MAGIC
...
# MAGIC
# MAGIC The LLM focuses on reasoning and communication. SDOL handles the data integrity.
...
# MAGIC ## Summary
# MAGIC
# MAGIC This notebook demonstrated the core building blocks of SDOL:
# MAGIC
...
# MAGIC
# MAGIC - **03_fleet_benchmark** -- Runs both a vanilla MCP agent and an SDOL-enhanced agent
# MAGIC   against the same questions, logging results to MLflow. Demonstrates the two failure
# MAGIC   modes that SDOL structurally solves: token-busting cross-paradigm joins and
